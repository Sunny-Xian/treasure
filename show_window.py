...
# 计算机与互联网图书销量榜地址
sales_volume_url = 'https://gw-e.jd.com/client.action?callback=func&body={{"moduleType":1,"page":{page},' \
                   '"pageSize":20,"scopeType":1,"month":0,' \
                   '"categoryFirst":"计算机与互联网"}}&functionId=bookRank&client=e.jd.com&_='
# 计算机与互联网图书热评排行榜地址
heat_rankings_url = 'https://gw-e.jd.com/client.action?callback=func&body={{"moduleType":2,"page":{page},' \
                    '"pageSize":20,"scopeType":1,"month":0,' \
                    '"categoryFirst":"计算机与互联网"}}&functionId=bookRank&client=e.jd.com&_='
...
class Main(QMainWindow, Ui_MainWindow):
    ...
    def get_attention_price(self, id):
        ...
        while True:
            if page == 6:
                break
            response = requests.get(sales_volume_url.format(page=page), headers=headers)  # 发送网络请求，获取服务器响应
...
class Price_Warning(QMainWindow, Price_Warning_MainWindow):
    ...
    def price(self):
        ...
        if len(attention_message) != 0:
            ...
            index = 0
            while True:
                if page == 6 or index >= len(attention_message):
                    break
                response = requests.get(sales_volume_url.format(page=page), headers=headers)  # 发送网络请求，获取服务器响应
                book_str = response.text.split('(', 1)[1].rsplit(')', 1)[0]  # 提取func()里面的信息
                book_dic = eval(book_str)  # 把字符串信息转换为字典信息
                ...
                for li in book_list:
                    # 京东价格
                    new_jd_price = li['sellPrice']
                    if attention_message[index][2] == li['bookName']:
                        ...
                        index += 1
                        if index >= len(attention_message):
                            break
                page += 1
            for i in range(len(attention_message)):
                for j in range(2):
